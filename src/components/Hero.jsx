  ...
  return (
    <section id="top" className="hero-section relative overflow-hidden bg-[#111812] text-white">
      <div className="relative min-h-[760px] md:min-h-[820px]">
        <div className="hero-media absolute inset-0" aria-label="Loleza Ridge Lodge photography slideshow">
          {slides.map((slide, index) => (
            <img
              key={slide.src}
              src={slide.src}
        ...
        <div className="absolute bottom-7 left-10 z-20 hidden text-[10px] uppercase tracking-[.24em] text-white/60 md:block">
          <span className="mr-3 inline-block h-px w-9 align-middle bg-[#d6ad4b]" /> Slow stays · Big views
        </div>
        <div className="absolute bottom-7 left-1/2 z-20 -translate-x-1/2">{controls}</div>
        <button onClick={() => scrollToSection('booking')} className="absolute bottom-7 right-10 z-20 hidden h-12 w-12 items-center justify-center rounded-full border border-white/25 bg-black/15 text-white/85 backdrop-blur transition hover:border-[#d6ad4b] hover:text-[#d6ad4b] md:flex" aria-label="Scroll to booking">↓</button>
      </div>
    </section>
  )
}
  ...
  return (
    <div className="hero-copy max-w-[720px]">
      <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .6 }} className="hero-eyebrow text-[#e0b84f] font-mono uppercase">
        Southern Highlands · Mbeya, Tanzania
      </motion.p>
