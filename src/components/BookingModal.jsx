...
export default function BookingModal({ initialBooking = null, onClose }) {
  ...
  return <div className="fixed inset-0 z-[100] bg-forest-950/75 backdrop-blur-md p-4 md:p-8 overflow-y-auto" onMouseDown={e => e.target === e.currentTarget && onClose()}>
    <div className="max-w-5xl mx-auto bg-parchment rounded-[2rem] shadow-2xl overflow-hidden min-h-[90vh]">
      <div className="flex items-center justify-between px-6 md:px-9 py-5 border-b border-ink/10">
        <div><p className="font-mono text-[10px] tracking-[.28em] uppercase text-clay">{t('Book direct')}</p><h2 className="font-display text-2xl md:text-3xl">{t('Plan your stay')}</h2></div>
      ...
      <form onSubmit={send} className="grid lg:grid-cols-[1.35fr_.8fr] gap-0">
        <div className="p-6 md:p-9 space-y-7">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div><label className="field-label"><Users /> {t('Adults')}</label><input type="number" min="1" max="10" name="adults" value={form.adults} onChange={update} className="field" /></div>
            <div><label className="field-label">{t('Children')}</label><input type="number" min="0" max="10" name="children" value={form.children} onChange={update} className="field" /></div>
          </div>
        ...
        <aside className="bg-forest-950 text-parchment p-6 md:p-9 flex flex-col justify-between">
          <div><p className="text-gold-400 font-mono text-[10px] tracking-[.25em] uppercase mb-5">{t('Booking summary')}</p><img src={room.img} alt={room.name} className="w-full h-44 object-cover rounded-2xl mb-5"/><h3 className="font-display text-2xl">{room.name}</h3><p className="text-parchment/60 text-sm mt-2">{room.size} · {room.beds} · {t('Up to')} {room.guests} {t('guests')}</p>
