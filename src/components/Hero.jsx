...
export default function Hero({ onBook }) {
  ...
  const controls = (
    <div className="flex items-center gap-2" aria-label="Hero slideshow controls">
  ...
  return (
    <section id="top" className="relative overflow-hidden bg-[#111812] text-white">
      {/* Desktop: preserve the wide editorial hero composition. */}
      <div className="hidden md:block relative min-h-[820px]">
        <div className="absolute inset-0" aria-label="Loleza Ridge Lodge photography slideshow">
          {slides.map((slide, index) => (
            <img key={slide.src} src={slide.src} alt={slide.alt} className={`hero-img absolute inset-0 w-full h-full object-cover transition-opacity duration-[1400ms] ease-in-out ${index === activeSlide ? 'opacity-100' : 'opacity-0'}`} />
          ))}
          <div className="hero-overlay absolute inset-0" />
          <div className="absolute inset-x-0 bottom-0 h-[48%] bg-gradient-to-t from-[#080906]/75 via-[#080906]/20 to-transparent" />
        </div>

        <div className="relative z-10 max-w-[1440px] mx-auto px-10 lg:px-14 pt-44 pb-44 min-h-[820px] flex items-center">
          <HeroCopy onBook={onBook} t={t} />
        </div>

        <div className="absolute bottom-7 left-10 lg:left-14 z-20 text-white/60 text-[10px] uppercase tracking-[.24em]"><span className="inline-block w-9 h-px bg-[#d6ad4b] mr-3 align-middle"/> Slow stays · Big views</div>
...
function HeroCopy({ onBook, t, mobile = false }) {
  return (
    <div className={mobile ? 'max-w-[680px]' : 'max-w-[720px]'}>
      ...
      <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .75, delay: .08 }} className="font-display text-white text-[3.05rem] sm:text-[4.5rem] md:text-[5.5rem] lg:text-[6.2rem] leading-[.9] tracking-[-.035em] drop-shadow-[0_4px_20px_rgba(0,0,0,.55)]">
