    ...
    <nav className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${solid || open ? 'bg-forest-950/96 backdrop-blur-xl shadow-lg shadow-black/15 border-b border-white/10' : 'bg-forest-950/70 backdrop-blur-md border-b border-white/5'}`}>
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="min-h-[76px] flex items-center gap-3 lg:gap-5">
          ...
          <div className="hidden 2xl:flex flex-1 min-w-0 items-center justify-center gap-x-3 2xl:gap-x-4 text-[10px] 2xl:text-[11px] tracking-[0.04em] text-parchment/80">
            {links.map(([id, label]) => (
          ...
          <div className="ml-auto shrink-0 flex items-center gap-2">
            <div className="hidden 2xl:flex items-center gap-1 rounded-full border border-white/10 p-1 bg-white/5" aria-label={t('Select language')}>
              {languages.map((lang) => (
            ...
            <button onClick={() => setOpen(v => !v)} className="2xl:hidden text-parchment w-10 h-10 flex items-center justify-center" aria-label={t('Menu')}>{open ? <X /> : <Menu />}</button>
          </div>
        </div>

        <AnimatePresence>
          {open && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="2xl:hidden overflow-hidden bg-forest-950/98 border-t border-white/10">
              <div className="px-2 pb-5 pt-3 grid grid-cols-2 sm:grid-cols-3 gap-x-3">
                {links.map(([id, label]) => <a key={id} href={`#${id}`} onClick={(e) => go(id, e)} className="py-3.5 px-3 border-b border-parchment/10 text-parchment/85 text-sm">{t(label)}</a>)}
                <div className="col-span-full flex items-center gap-2 mt-4 px-3">
                  <span className="text-xs text-parchment/50 mr-1">{t('Language')}</span>
                  {languages.map(lang => <button key={lang.code} type="button" onClick={() => setLanguage(lang.code)} className={`px-3 py-1.5 rounded-full text-xs ${language === lang.code ? 'bg-gold text-forest-950' : 'border border-parchment/15 text-parchment/70'}`}>{lang.short}</button>)}
                </div>
