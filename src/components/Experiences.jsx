  ...
  <div className="max-w-7xl mx-auto px-6 md:px-10">
   ...
   <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
    {experiences.map((e,i)=>{const Icon=e.icon;return <motion.article key={e.title} initial={{opacity:0,y:22}} whileInView={{opacity:1,y:0}} viewport={{once:true,amount:.12}} transition={{delay:i*.06}} className="group overflow-hidden rounded-[1.35rem] border border-ink/10 bg-white shadow-[0_14px_40px_rgba(35,27,19,.05)] hover:-translate-y-1.5 hover:shadow-[0_22px_50px_rgba(35,27,19,.10)] transition-all">
      <div className="relative h-48 overflow-hidden"><img src={e.image} alt={t(e.title)} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-black/55 to-transparent"/><span className="absolute left-5 bottom-5 w-10 h-10 rounded-full bg-[#fffdf8]/90 text-[#9d7428] flex items-center justify-center"><Icon className="w-5" strokeWidth={1.5}/></span></div>
