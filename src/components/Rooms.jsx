...
import { rooms, roomsIntro } from '../config'
...
export default function Rooms({ onBook }) {
  ...
  return <section id="rooms" className="scroll-mt-24 py-24 md:py-32 bg-parchment"><div className="max-w-7xl mx-auto px-6 md:px-10">
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14"><div><p className="eyebrow text-clay">{t(roomsIntro.eyebrow)}</p><h2 className="font-display text-4xl md:text-6xl leading-[1.02]">{t(roomsIntro.title)}<br className="hidden md:block"/> {t(roomsIntro.titleAccent)}</h2></div><p className="text-ink/60 max-w-md leading-relaxed">{t(roomsIntro.description)}</p></div>
